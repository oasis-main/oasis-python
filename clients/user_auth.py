...
#Using post requests and encrypted bodies for everything is supposedly not best practice
#But aside from lack of readability for robots,
#I can't find that it introduces any real disadvantages,
#And it makes it SO. MUCH. EASIER. to structure requests consistently accross multiple tls interfaces (https, websockets, mqtts, etc. )

def create_new_user(email: str, password: str, admin_user_id: str, group_name: str):
    data = {"email": email, "password": password, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/create_account/', json = data)
    #print(r.content)
    try:
...
def change_password(email: str, admin_user_id: str, group_name: str):
    data = {"email": email, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/change_password/' , json = data)
    try:
...
def password_login(email: str, password: str, admin_user_id: str, group_name: str, return_tokens = False):
    data = {"email": email, "password": password, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/login/password/', json = data)
    try:
...
def get_session(refresh_token: str, admin_user_id: str, group_name: str, return_tokens = False):
    data = {"refresh_token": refresh_token, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/login/session/', json = data)
    try:
...
def verify_session(user_id: str, id_token: str, admin_user_id: str, group_name: str):
    data = {"user_id": user_id, "id_token": id_token, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/verify_session/' , json = data)
    try:
...
def request_user_access(user_id: str, id_token: str, admin_user_id: str, group: str, data_type: Literal["allowance_count","credit_balance","clearance_code"], field: str, required: str, emissions_kg: Optional[float] = 0.0000, check_only: Optional[bool] = False):
    data = {"user_id": user_id, "id_token": id_token, "admin_user_id": admin_user_id, "group": group, "data_type": data_type, "field": field, "required": required, "emissions_kg": emissions_kg, "check_only": check_only}
    r = httpx.post(url +'/user/access_request/', json = data)
    try:
...
def get_id_from_email(user_email: str, id_token: str, admin_user_id: str, group_name: str):
    data = {"user_email": user_email, "id_token": id_token, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/get_uid_by_email/', json = data)
    try:
...
def get_email_from_id(user_id: str, id_token: str, admin_user_id: str, group_name: str):
    data = {"user_id": user_id, "id_token": id_token, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/get_email_by_uid/', json = data)
    try:
...
def delete_user(user_id: str, password: str, admin_user_id: str, group_name: str):
    data = {"user_id": user_id, "password": password, "admin_user_id": admin_user_id, "group_name": group_name}
    r = httpx.post(url +'/user/delete_account/' , json = data)
    try:
        attempt_result = orjson.loads(r.content)
        attempt_result.update({"url": str(r.url)})
        return attempt_result # The name of the return variable should tell us how to parse the resulting dictionary
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "test_delete":
        ...
        print("Checking old id_token status... (should fail)")
        result = verify_session(user_id, id_token, admin_user_id, "oasis-users")
        print(result)

        print("Logging-in with deleted username & password... (should fail)")
